...
// 烈焰风暴 - 群体法术攻击，对所有敌人造成伤害
// 拖拽方式与技能、能力卡相同（向上拖拽使用）
public class FlamestrikeCard : SpellAttackCard
{
    // ========== AOE卡牌通用逻辑 ==========
    ...
    private void UpdateAOEDamagePreview()
    {
        ...
        // 计算总伤害：基础伤害(含亲和度) + 消耗亲和度加成，复制药水加第二段(无亲和度)
        int fireAffinity = BuffManager.Instance.GetStack(StatusType.FireAffinity);
        int baseDamage = GetAttackDamage(); // 含火亲和度
        int rawBase = GetArg0(); // 不含任何加成的原始伤害
        int bonusPerAffinity = IsUpgraded() ? 2 : 1;
        int totalDamage = baseDamage + fireAffinity * bonusPerAffinity;
        if (BuffManager.Instance.HasStatus(StatusType.Duplicate))
            totalDamage += rawBase + 0 * bonusPerAffinity; // 第二段无火亲和度
        ...
        // 检查鼠标是否指向敌人区域（简化处理：使用第一个敌人位置或屏幕中心）
        Vector3 previewPos;
        ...
        fightUI.ShowDamagePreview(totalDamage, previewPos);
    }
    ...
    protected override void OnCardUsed()
    {
        ...
        // 先计算基础伤害（包含当前火亲和度加成），再消耗
        int fireAffinity = BuffManager.Instance.GetStack(StatusType.FireAffinity);
        int baseDamage = GetAttackDamage(); // 此时已含火亲和度加成
        if (fireAffinity > 0)
        {
            BuffManager.Instance.RemoveStatus(StatusType.FireAffinity, fireAffinity);
        }

        // 总伤害 = 基础（含亲和度） + 消耗亲和度额外加成（升级后每层+2）
        int bonusPerAffinity = IsUpgraded() ? 2 : 1;
        int totalDamage = baseDamage + fireAffinity * bonusPerAffinity;

        // 对所有存活敌人造成伤害
        StartCoroutine(DealDamageToAllEnemies(totalDamage, effectPath));
    }
